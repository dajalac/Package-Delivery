...
def truck_out_for_delivery(priority_vertices_list, eob_vertices_list, truck_number, time):
    ...
    priority_packages_shortPath = distances.get_shortPath()
    priority_packages_time = set_time(priority_packages_shortPath, truck_number, time, is_priority=True)
    #
    # # for packages with EOD deadline
    eob_endPoint, eob_distance = distances.find_short_path(eob_vertices_list, True,
                                                           priority_endPoint)  # true = go_to_hub
    eob_package_short_path = distances.get_shortPath()
    eob_packages_time = set_time(eob_package_short_path, truck_number, priority_packages_time, is_priority=False)
    #
